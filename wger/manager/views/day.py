...
class DayEditView(DayView, UpdateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if 'Microcycle' in workout.cycle:
            ...
            context['form'].fields['day'].queryset = DaysOfWeek.objects.filter(the_type='day')
        elif 'Mesocycle' in workout.cycle:
            context['form'].fields['day'].label = 'Week'
            context['form'].fields['day'].queryset = DaysOfWeek.objects.filter(the_type='week')
        elif 'Macrocycle' in workout.cycle:
            context['form'].fields['day'].label = 'Month'
            context['form'].fields['day'].queryset = DaysOfWeek.objects.filter(the_type='month')
    ...
    def get_context_data(self, **kwargs):
        ...
        if 'Microcycle' in workout.cycle:
            ...
            context['form'].fields['day'].queryset = DaysOfWeek.objects.filter(
                the_type='day').exclude(day_of_week__in=already_selected_option_names)
        elif 'Mesocycle' in workout.cycle:
            ...
            context['form'].fields['day'].label = 'Week'
            context['form'].fields['day'].queryset = DaysOfWeek.objects.filter(
                the_type='week').exclude(day_of_week__in=already_selected_option_names)
        elif 'Macrocycle' in workout.cycle:
            context['form'].fields['day'].label = 'Month'
            context['form'].fields['description'].help_text = _('A description of what is done on this month (e.g. '
                                    ...
                                    '"Arms and abs")')
            context['form'].fields['day'].queryset = DaysOfWeek.objects.filter(
                the_type='month').exclude(day_of_week__in=already_selected_option_names)
